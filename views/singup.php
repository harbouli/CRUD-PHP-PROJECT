...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $check_email_stmt = $pdo->prepare($check_email);
    ...
    elseif ($check_email_stmt->rowCount()) :
        $erorrs[] = 'This Email Is All Ready Exist';
    elseif (empty($password)) :
        $erorrs[] = 'Please Enter Your email';
    elseif (strlen($password) < 8) :
        $erorrs[] = 'Your Password Is Less Than 8 Caracters';
    elseif (empty($email)) :
        $erorrs[] = 'Please Enter Your email';
    elseif (empty($erorrs)) :

        try {
            $statemnt = $pdo->prepare("INSERT INTO user_info (firstname , lastname , password, email) VALUES (:firstname , :lastname ,:password, :email)");


            $statemnt->bindValue(':firstname', $firstname);
            $statemnt->bindValue(':lastname', $lastname);
            $statemnt->bindValue(':password', password_hash($password, PASSWORD_DEFAULT));
            $statemnt->bindValue(':email', $email);

            $statemnt->execute();
            header("location: login.php");
        } catch (PDOException $e) {
            $erorrs[] = 'something went wrong';
        }
    endif;
}
?>
...
<body>
    <div class="container">
        <div class="card">
            ...
            <p class="card_title-info">Wellcom</p>
            <form class="card_form" method="post" action="">
                <div class="input">
                    <input name="lastname" type="text" value="<?php echo $lastname ?>" class="input_field" required />
                    <label class="input_label">Lastname</label>
                </div>
                <div class="input">
                    <input name="firstname" type="text" value="<?php echo $firstname ?>" class="input_field" required />
                    <label class="input_label">Firstname</label>
                </div>
                <div class="input">
                    <input name="email" type="text" value="<?php echo $email ?>" class="input_field" required />
                    <label class="input_label">Email</label>
                </div>
                <div class="input">
                    <input name="password" type="password" value="<?php echo $password ?>" class="input_field" required />
